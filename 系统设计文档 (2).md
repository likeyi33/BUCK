...
最小系统板的插座、ir2103直插式芯片及其插座、470nf电容：预计七月二号到
...
PID控制器根据目标值与实际值的偏差，经比例、积分、微分运算后生成控制量，动态调整PWM占空比，使输出电压跟随目标值。
...
本设计参数：$K_p=0.8$，$K_i=0.05$，$K_d=0.02$。

\*\*（3）参数整定\*\*

试凑法整定：先调$K_p$至系统微振荡，再增$K_i$消除静差，最后加$K_d$抑制超调。

\*\*（4）占空比更新\*\*

$$D(k) = 0.80 + u(k)/100$$

限幅至10%~90%后，更新PWM比较值 $CCR = D \\times 720$。
...
$$V_{OUT} = \\frac{N_{ADC} \\times 4.125}{4096}$$

\#### 3. PWM占空比控制算法

Buck电路CCM模式下：$V_{OUT} = D \\cdot V_{IN}$，理论占空比 $D = 12/15 = 0.80$。

定时器 $ARR=719$，PWM比较值 $CCR = D \\times 720$，初始 $CCR = 576$。

为防止异常，将占空比限幅在10%~90%，即 $CCR \\in \[72, 648\]$。
...
$$f_{PWM} = \\frac{72\\text{MHz}}{(0+1) \\times (719+1)} = 100\\text{kHz}$$
